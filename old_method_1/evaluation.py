...
class ModelEvaluator:
    ...
    def visualize_predictions(self, num_samples=8, save_dir='./visualizations'):
        ...
        # Color map for segmentation - add extra color for label 255
        colors = np.zeros((256, 3))
        colors[:self.num_classes] = plt.cm.tab20(np.linspace(0, 1, self.num_classes))[:, :3] * 255
        colors[255] = [0, 0, 0]  # Black for ignore/unlabeled
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                for i in range(min(batch_size, num_samples - sample_count)):
                    ...
                    axes[1, 2].set_title(f'Errors ({error_rate:.1f}%)')
                    axes[1, 2].axis('off')
